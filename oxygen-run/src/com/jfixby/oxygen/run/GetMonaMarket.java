...
public class GetMonaMarket {

	public static void main (final String[] args) throws IOException {
		ScarabeiDesktop.deploy();
		SystemSettings.setExecutionMode(ExecutionMode.DEMO);
		final NicehashOrders market = GetNicehashMarket.getOrders(Algo.LYRA, Region.resolve("EU"), SpeedMagnitude.GHpS);
		L.d("nicehash", market.result.orders);

		final StratumPoolPerformance pool = new MonaHubPoolPerformance().read();

		final Ticker btcdcr = GetTicker.get(pool.perFlowUnitX24H().coinsX24H().sign);
