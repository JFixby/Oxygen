...
package com.jfixby.oxygen.dcr.call.pool;
...
public class DCRPoolPerformance implements Pool {
	public static final String API_CALL = "https://www2.coinmine.pl/dcr/index.php";
...
// e.printStackTrace();
	...
	private static void extract (final String html, final StratumPoolPerformance performance) throws IOException {
		...
		}
		{
			...
			final List<String> trList = Strings.split(tableContent, "<tr>").filter( (x) -> x.length() > 0);
			for (final String line : trList) {
// L.d("line", line);
				final List<String> lines = Strings.split(line, "</tr>").filter( (x) -> x.length() > 0);
// L.d(" ", lines);
				splits.addAll(lines);
			}
			if (trList.size() < 10) {
...
// L.d("data", data);
					...
					if (i == 3) {
						final double KhashesPsec = Double.parseDouble(data.replaceAll(",", ""));
						final SpeedMagnitude order = SpeedMagnitude.THpS;
						final Speed speed = Speed.newValue(KhashesPsec, order);
						user.speed = speed;

					}
					if (i == 4) {
						user.coinsPerDay = ShitCoin.newCoin(Double.parseDouble(data.replaceAll(",", "")), CoinSign.DECRED);
					}
					if (i == 5) {
